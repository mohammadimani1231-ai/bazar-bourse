 ...
 * در دسترس، هیچ‌وقت کامل مخفی نمی‌شود. پس‌زمینه/رنگ‌ها طبق design_handoff_dashboard_redesign:
...
export function Sidebar({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <aside className="sticky top-0 flex h-dvh w-16 shrink-0 flex-col border-l border-border bg-surface lg:w-56">
      <Link
        href="/"
        className="flex h-16 shrink-0 items-center justify-center gap-2 border-b border-border px-2 text-sm font-bold text-foreground lg:justify-start lg:px-4"
      >
        ...
        {links.map((link) => {
          ...
          return (
            <Link
              ...
              href={link.href}
              title={link.label}
              className={`flex items-center justify-center gap-3 rounded-lg border px-2 py-2.5 text-sm transition-colors lg:justify-start lg:px-3 ${
                active
                  ? "border-accent bg-accent-soft font-bold text-accent"
                  : "border-transparent font-medium text-muted hover:border-accent hover:text-accent"
              }`}
            >
